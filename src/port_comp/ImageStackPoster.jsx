...
export default function ImagestackProduct() {
  ...
  return (
    <div className="p-4 mx-auto w-fit items-center justify-center text-center">
      {/* Stacked Preview */}
      <div className="relative flex justify-center items-center h-32 pb-20">
            {images.slice(0, 3).map((src, index) => {
                ...
                <img
                    ...
                    src={src}
                    alt={`Preview ${index}`}
                    className={`absolute w-32 h-32 lg:w-44 lg:h-44 rounded-lg border-2 border-white shadow-lg transform ${rotation} ${translate}`}
                    style={{ zIndex }}
                />
                );
            })}
      ...
      {open && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center w-screen  lg:w-full z-50">
            <div className="relative bg-transparent  backdrop-blur rounded-lg lg:mx-20 w-68 lg:w-full p-6 overflow-y-auto h-[800px]">
            <button
